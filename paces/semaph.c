...
again:
        ...
        if (semval == 0) {
                ...
                if (semctl(id, 0, SETVAL, semctl_arg) < 0)
                        system_error("can't SETVAL[0]");

                semctl_arg.val = BIGCOUNT;
                if (semctl(id, 1, SETVAL, semctl_arg) < 0)
                        system_error("can't SETVAL[1]");
        }
...
sem_close(int id)
{
        ...
        if (semval > BIGCOUNT)
                system_error("sem[1] > BIGCOUNT");
        else if (semval == BIGCOUNT)
                sem_rm(id);
        else
                if (semop(id, &op_unlock[0], 1) < 0)
                        system_error("can't unlock");   /* unlock */
}
...
sem_op(int id, int value)
{
        if ( (op_op[0].sem_op = value) == 0)
                system_error("can't have value == 0");

        if (semop(id, &op_op[0], 1) < 0)
                system_error("sem_op error");
}
